...
%% Phase plane grid and vector field
vmin = -20;  vmax = 120;
mmin = 0;    mmax = 1;
...
%% Steady states as intersections of nullclines
Froot = @(m) m_inf( v_nc_fun(m) ) - m;
...
% Vector field as a function of (v,m) only
Fvec = @(v,m) [ ...
    (-gK*n0^4.*(v - vK) ...
     -gNa.*(m.^3).*h0.*(v - vNa) ...
     -gL.*(v - vL)) / Cm; ...
...
% Numerical Jacobian at (v,m)
epsJ = 1e-6;
numJ = @(v,m) [ ...
    (Fvec(v+epsJ,m) - Fvec(v-epsJ,m))/(2*epsJ), ...
    (Fvec(v,m+epsJ) - Fvec(v,m-epsJ))/(2*epsJ) ];

% Find saddle equilibrium (mixed-sign eigenvalues)
idx_saddle = [];
for k = 1:numel(v_eq)
    Jk  = numJ(v_eq(k), m_eq(k));
    lam = eig(Jk);
    if real(lam(1))*real(lam(2)) < 0
        idx_saddle = k;
        break
    end
end
...
% Stable eigenvector of the saddle
Js = numJ(v_s,m_s);
[vecs,vals] = eig(Js);
lam = diag(vals);
[~,i_stable] = min(real(lam));   % most negative eigenvalue
e_s = vecs(:,i_stable);
e_s = e_s / norm(e_s);

% Start a little away from the saddle along ±e_s
eps0 = 1e-3;
IC1 = [v_s; m_s] + eps0*e_s;
IC2 = [v_s; m_s] - eps0*e_s;

% Integrate backward in time with stopping when leaving the window
tspan_man = [0 -80];   % shorter to avoid wild blow-up
opts = odeset('RelTol',1e-8,'AbsTol',1e-10, ...
              'Events',@(t,y) stop_outside(t,y,vmin,vmax,mmin,mmax));

[t1,y1] = ode45(f, tspan_man, IC1, opts);
[t2,y2] = ode45(f, tspan_man, IC2, opts);

% Plot stable manifold branches
...
legend({'vector field','v-nullcline','m-nullcline', ...
        'steady states','stable manifold'}, ...
       'Location','bestoutside');

%% Local event function (must be at end of file)
function [value,isterminal,direction] = stop_outside(t,y,vmin,vmax,mmin,mmax)
    v = y(1); m = y(2);
    % Stop when v or m leaves the plotting window
    value = [v - vmin; vmax - v; m - mmin; mmax - m];
    isterminal = [1; 1; 1; 1];
    direction  = [0; 0; 0; 0];
end
